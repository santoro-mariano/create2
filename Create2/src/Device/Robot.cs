namespace Create2.Device
{
    ...
    public class Robot
    {
        ...
        private bool CheckStarted(OpCode code)
        {
            if (!this.IsStarted)
            {
                this.logger.LogError($"Robot must be started to execute command (OpCode: {code}, Current Mode: {this.CurrentMode})");
            }
        ...
        private bool CheckActive(OpCode code)
        {
            if (!this.IsActive)
            {
                this.logger.LogError($"Robot must be active to execute command (OpCode: {code}, Current Mode: {this.CurrentMode})");
            }
